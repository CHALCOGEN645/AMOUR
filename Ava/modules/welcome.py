...
from Ava.modules.helper_funcs.chat_status import user_admin as u_admin
from Ava.modules.helper_funcs.decorators import Avacallback, Avacmd, Avamsg
from Ava.modules.helper_funcs.misc import build_keyboard, revert_buttons
from Ava.modules.helper_funcs.msg_types import get_welcome_type
from Ava.modules.helper_funcs.string_handling import (
...
@Avamsg((Filters.status_update.new_chat_members), group=WELCOME_GROUP)
@loggable
    ...
    for new_mem in new_members:
        ...
        if should_welc:
            ...
            if new_mem.id == OWNER_ID:
                ...
                update.effective_message.reply_text(
                    f"ᴛʜᴀɴᴋꜱ ꜰᴏʀ ᴀᴅᴅɪɴɢ ᴍᴇ! ᴊᴏɪɴ @{penguin_x_support} ꜰᴏʀ ꜱᴜᴘᴘᴏʀᴛ.",
                    reply_to_message_id=reply,
                )
                continue
...
@Avamsg((Filters.status_update.left_chat_member), group=WELCOME_GROUP)
...
@Avacallback(pattern=r"user_join_")
...
@Avacallback(pattern=r"user_captchajoin_\([\d\-]+,\d+\)_\(\d{4}\)")
...
@Avacmd(command="welcomehelp")
@u_admin
def welcome_help(update: Update, context: CallbackContext):
    update.effective_message.reply_text(WELC_HELP_TXT, parse_mode=ParseMode.MARKDOWN)


@Avacmd(command="welcomemutehelp")
@u_admin
